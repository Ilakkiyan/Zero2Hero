...
npm run llm:pull                  # ollama pull qwen2.5:7b  (~4.7 GB, one time)
...
`qwen2.5:7b`). `.env.local` is git-ignored — never commit real keys.

### 3. Pull the local model

```bash
npm run llm:pull        # = ollama pull qwen2.5:7b
```
...
| Banner says "pull the model" | `npm run llm:pull` (or `ollama pull qwen2.5:7b`). |
