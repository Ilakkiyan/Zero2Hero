 ...
 * provider (local Ollama by default). The SEARCH step reaches the web via a
 * local, fully private SearxNG instance — no API key, no cloud dependency.
 *
...
export async function* runAgenticResearch(
  ...
  // 1. PLAN (local brain)
  const questions = await planQuestions(brief, opts.provider, opts.model);
  ...
  for (let i = 0; i < questions.length; i++) {
    ...
    try {
      const { text, sources } = await searxSearch(questions[i], searxUrl);
